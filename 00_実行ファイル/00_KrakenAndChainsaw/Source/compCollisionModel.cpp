...
// 静的メンバー変数宣言
bool CCollisionModel::m_bDisp = true;	// 描画の有無
...
// 当たり判定用モデルコンストラクタ
//****************************************************
CCollisionModel::CCollisionModel(const int nIdxBone, const D3DXVECTOR3 pos, const D3DXQUATERNION quat, const std::string& XFilePath)
	: m_nIdxBone(nIdxBone)
	, m_pos(pos)
	, m_quat(quat)
...
// 当たり判定用モデル初期化
//****************************************************
void CCollisionModel::Init(void)
{
	// デバイスの取得
...
void CCollisionModel::Draw(void)
{
	...
	for (int nCntMat = 0; nCntMat < (int)m_dwNumMat; nCntMat++)
	{
		...
		// テクスチャの設定
		pDevice->SetTexture(0, pTexture->GetAddress(m_vsTexturePath[nCntMat].c_str()));
...
// 当たり判定用モデル回転
//****************************************************
void CCollisionModel::AddRotate(D3DXVECTOR3 axis, float fAngle)
{
	// 加えるクォータニオン
