...
// 矩形と矩形の当たり判定
//**************************************************
bool CMyMath::IsColiisionLectAndPoint(D3DXVECTOR2 posA, float fWidthA, float fHeightA, D3DXVECTOR2 posB)
{
	float fLeft = posA.x - fWidthA;
	float fRight = posA.x + fWidthA;
	float fTop = posA.y - fHeightA;
	float fBottom = posA.y + fHeightA;

	// 四角の範囲内に入っていれば当たっているとみなす
	if (posB.x > fLeft && posB.x < fRight &&
		posB.y > fTop && posB.y < fBottom)
	{
		return true;
	}
