...
HRESULT CJumpGuide::Init(void)
{
	...
	Polygon2DIconArg.TexturePath	= "data\\texture\\JumpGuide.png";
...
void CJumpGuide::Update(void)
{
	...
	// ジャンプしたら
	if (
		pKeyboard->GetPress(DIK_SPACE) == true ||
		pJoyPad->GetkeyPress(CInputJoypad::JOYKEY_A) == true
		)
	{
		// カウンターを減らすようにする
	...
	if (pCounter->GetIsMin() == true)
	{
		...
		// シーンから削除
		CManager::GetInstance()->GetScene()->DeleteObj<CJumpGuide>();
	}
}
...
// 生成
//***********************************************
CJumpGuide* CJumpGuide::Create(void)
{
	// インスタンス生成と初期化
	CJumpGuide* pJumpGuide = new CJumpGuide;
	pJumpGuide->Init();
